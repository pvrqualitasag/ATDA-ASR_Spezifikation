...
Da die Umsetzung dieses Projektes sehr stark von den Gegebenheiten, wie sie bei den Anbietern der Melkanlagen herrschen, abhängen, ist diese Spezifikation in gewissen Abschnitten noch nicht sehr konkret gefasst. Dies ist auf der einen Seite eine bewusste Entscheidung, welche die Offenheit und die Flexibilität von unserer Seite her demonstrieren soll. Auf der anderen Seite ist das erste Informationstreffen mit den wichtigsten Anbietern von Melkanlagen auf den 18. März 2016 angesetzt. Vor diesem Treffen fehlen uns schlicht und einfach viele Informationen, welche für eine konkretere Ausformulierung dieser Spezifikation, nötig wären. 
...
objSecEnum$displayNumSection()
```
Die Standardprozedur für den Transfer von Milchleistungsdaten bei Betrieben, welche sich bei der Milchleistungsprüfung der Zuchtverbände (BVCH und SHB) beteiligen, sieht so aus, dass ein Milchkontrolleur monatlich Milchproben und gemessene Milchmengen von einem Betrieb abholt. Die Milchmengen werden während der Kontrolle auf dem Betrieb manuell auf vorgedruckte Formulare eingetragen. Die ausgefüllten Formulare werden dann an die Zuchtorganisationen geschickt und dort per Scanner in das Informationssystem (ARGUS) von Qualitas eingelesen. Das nachfolgende Diagramm versucht den Datenfluss anlässlich einer Milchkontrolle auf einem Betrieb schematisch darzustellen. Der Datenfluss verläuft nach der Reihenfolge der Nummern. Die Bedeutung der einzelnen Nummern wird später noch erklärt werden.
...
objSecEnum$displayNumSection()
```
Das folgende Diagramm gibt eine ungefähre Aufteilung der verschiedenen Themen auf die Zeit der ersten Hälfte des Jahres 2016. Eine Planung darüber hinaus ist vor dem ersten Treffen mit den Anbietern der Melkanlagen sehr schwierig.
